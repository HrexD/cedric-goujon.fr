...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("INSERT INTO candidatures (poste, lieu, site_annonce, url_annonce, date_candidature, entreprise, remuneration) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->execute([
        $poste,
        $lieu,
        $site_annonce,
...
<div class="container mt-5">
    <h2 class="mb-4">Ajouter une candidature</h2>

    <form method="post" class="bg-white p-4 shadow-sm rounded">
        <div class="mb-3">
            <label for="poste" class="form-label">Intitulé du poste</label>
            <input type="text" name="poste" id="poste" list="liste_poste" class="form-control" required autocomplete="off">
            <datalist id="liste_poste"></datalist>
        </div>
        <div class="mb-3">
            <label for="entreprise" class="form-label">Entreprise</label>
            <input type="text" name="entreprise" id="entreprise" list="liste_entreprise" class="form-control" required autocomplete="off">
            <datalist id="liste_entreprise"></datalist>
        </div>
        <div class="mb-3">
            <label for="lieu" class="form-label">Lieu</label>
            <input type="text" name="lieu" id="lieu" list="liste_lieu" class="form-control" required autocomplete="off">
